...
window.LCalendar = (function() {
    var MobileCalendar = function() {
        ...
        this.maxM = 12;
        this.maxD = 31;
        this.topH = 2;  //默认列表离顶部的高度(使用rem单位时是2，使用em单位时是8)
        this.lHeight = 1;  //列表第一行的高度(使用rem单位时是1rem，使用em单位时是1em)
    }
        ...
        bindEvent: function(type) {
            ...
            function setDateGearTooth() {
                ...
                if (date_yy && date_yy.getAttribute("val")) {
                    ...
                    var top = Math.floor(parseInt(date_yy.getAttribute('top')));
                    if (!isNaN(top)) {
//                      top % 2 == 0 ? (top = top) : (top = top - 1);
                        top > _self.topH && (top = _self.topH);
                        var minTop = _self.topH - (passY - 1) * _self.lHeight;
                        top < minTop && (top = minTop);
                        date_yy.style["-webkit-transform"] = 'translate3d(0,' + top + 'rem,0)';
                        date_yy.setAttribute('top', top + 'rem');
                        yyVal = Math.abs(top - _self.topH) / _self.lHeight;
                        date_yy.setAttribute("val", yyVal);
                    } else {
                        date_yy.style["-webkit-transform"] = 'translate3d(0,' + (_self.topH - yyVal * _self.lHeight) + 'rem,0)';
                        date_yy.setAttribute('top', _self.topH - yyVal * _self.lHeight + 'rem');
                    }
                } else {
                    return;
                }
                ...
                if (date_mm && date_mm.getAttribute("val")) {
                    ...
                    date_mm.style["-webkit-transform"] = 'translate3d(0,' + (_self.topH - (mmVal - minM) * _self.lHeight) + 'rem,0)';
                    date_mm.setAttribute('top', _self.topH - (mmVal - minM) * _self.lHeight + 'rem');
                } else {
                    return;
                }
                var date_dd = _self.gearDate.querySelector(".date_dd");
                if (date_dd && date_dd.getAttribute("val")) {
                    ...
                    date_dd.style["-webkit-transform"] = 'translate3d(0,' + (_self.topH - (ddVal - minD) * _self.lHeight) + 'rem,0)';
                    date_dd.setAttribute('top', _self.topH - (ddVal - minD) * _self.lHeight + 'rem');
                } else {
                    return;
                }
            }
            ...
            function setTimeGearTooth() {
                ...
                if (time_hh && time_hh.getAttribute("val")) {
                    ...
                    time_hh.innerHTML = i;
                    time_hh.style["-webkit-transform"] = 'translate3d(0,' + (_self.topH - hhVal * _self.lHeight) + 'rem,0)';
                    time_hh.setAttribute('top', _self.topH - hhVal * _self.lHeight + 'rem');
                } else {
                    return
                }
                var time_mm = _self.gearDate.querySelector(".time_mm");
                if (time_mm && time_mm.getAttribute("val")) {
                    ...
                    time_mm.innerHTML = i;
                    time_mm.style["-webkit-transform"] = 'translate3d(0,' + (_self.topH - mmVal * _self.lHeight) + 'rem,0)';
                    time_mm.setAttribute('top', _self.topH - mmVal * _self.lHeight + 'rem');
                } else {
                    return
                }
            }
            ...
            function rollGear(target) {
                ...
                target["int_" + target.id] = setInterval(function() {
                    ...
                    if (Math.abs(speed) > 0.1) {} else {
                        var b = Math.round(pos / _self.lHeight) * _self.lHeight;
                        pos = b;
                        setDuration();
                    }
                    if (pos > _self.topH) {
                    ...
                    switch (target.dataset.datetype) {
                        case "date_yy":
                            ...
                            if (stopGear) {
                                var gearVal = Math.abs(pos - _self.topH) / _self.lHeight;
                        ...
                        case "date_mm":
                            ...
                            var minTop = _self.topH - (maxM - minM) * _self.lHeight;
                            if (pos < minTop) {
                                pos = minTop;
                                setDuration();
                            }
                            if (stopGear) {
                                var gearVal = Math.abs(pos - _self.topH) / _self.lHeight + minM;
                                setGear(target, gearVal);
                                clearInterval(target["int_" + target.id]);
                            }
                            break;
                        case "date_dd":
                            ...
                            var minTop = _self.topH - (maxD - minD) * _self.lHeight;
                            if (pos < minTop) {
                                pos = minTop;
                                setDuration();
                            }
                            if (stopGear) {
                                var gearVal = Math.abs(pos - _self.topH) / _self.lHeight + minD;
                                setGear(target, gearVal);
                                clearInterval(target["int_" + target.id]);
                            }
                            break;
